...
public class SchedulerFactoryBean extends SchedulerAccessor implements FactoryBean<Scheduler>,
	...
	public void afterPropertiesSet() throws Exception {
		...
		/**
		 * 在prepareSchedulerFactory 方法中 schedulerFactoryClass是StdSchedulerFactory,prepareSchedulerFactory方法中会创建一个StdSchedulerFactory对象
		 *
		 */
	...
	private Scheduler prepareScheduler(SchedulerFactory schedulerFactory) throws SchedulerException {
		...
		try {
			/**
			 * 这里使用SchedulerFactory来创建Scheduler对象
			 *
			 */
	 ...
	 *
	 *
	 *  该方法的调用时机是 afterPropertiesSet方法中，因为当前类实现了InitializingBean接口，所以afterPropertiesSet方法会被调用
	 *
	...
	protected Scheduler createScheduler(SchedulerFactory schedulerFactory, @Nullable String schedulerName)
		...
		try {
			/**
			 * 首先我们注意到SchedulerRepository内存存在一个map schedulers存放 schedulername 和scheduler之间的关系
			 * SchedulerRepository构造器私有 通过getInstance对外提供实例，在jvm内仅有这一个实例。
			 *SchedulerRepository中 的方法都是Synchronized
			 */
			SchedulerRepository repository = SchedulerRepository.getInstance();
			synchronized (repository) {
				/**
				 * 先从map中查，如果没有则使用schedulerFactory的getScheduler方法创建Scheduler
				 */
				Scheduler existingScheduler = (schedulerName != null ? repository.lookup(schedulerName) : null);
				/**
				 * 使用SchedulerFactory的getScheduler方法获取Scheduler，
				 * 在getScheduler中首先也是从SchedulerRepository的map中查询一下，如果不存在则 创建
				 */
